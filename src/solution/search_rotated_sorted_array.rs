use crate::*;
use std::cmp::Ordering;

impl Solution {
    /// # [33. Search in Rotated Sorted Array](https://leetcode.com/problems/search-in-rotated-sorted-array/description/?envType=study-plan-v2&envId=top-interview-150)
    ///
    /// There is an integer array `nums` sorted in ascending order (with **distinct**  values).
    ///
    /// Prior to being passed to your function, `nums` is **possibly rotated**  at an unknown pivot index `k` (`1 <= k < nums.length`) such that the resulting array is `[nums[k], nums[k+1], ..., nums[n-1], nums[0], nums[1], ..., nums[k-1]]` (**0-indexed** ). For example, `[0,1,2,4,5,6,7]` might be rotated at pivot index `3` and become `[4,5,6,7,0,1,2]`.
    ///
    /// Given the array `nums` **after**  the possible rotation and an integer `target`, return the index of `target` if it is in `nums`, or `-1` if it is not in `nums`.
    ///
    /// You must write an algorithm with `O(log n)` runtime complexity.
    ///
    /// **Example 1:**
    ///
    /// ```txt
    /// Input: nums = [4,5,6,7,0,1,2], target = 0
    /// Output: 4
    /// ```
    ///
    /// **Example 2:**
    ///
    /// ```txt
    /// Input: nums = [4,5,6,7,0,1,2], target = 3
    /// Output: -1
    /// ```
    ///
    /// **Example 3:**
    ///
    /// ```txt
    /// Input: nums = [1], target = 0
    /// Output: -1
    /// ```
    ///
    /// **Constraints:**
    ///
    /// - `1 <= nums.length <= 5000`
    /// - `-10^4 <= nums[i] <= 10^4`
    /// - All values of `nums` are **unique** .
    /// - `nums` is an ascending array that is possibly rotated.
    /// - `-10^4 <= target <= 10^4`
    pub fn search_rotated_sorted_array(nums: Vec<i32>, target: i32) -> i32 {
        Self::search_rotated_sorted_array_recursive(&nums, target, 0, nums.len())
    }

    fn search_rotated_sorted_array_recursive(
        nums: &[i32],
        target: i32,
        start: usize,
        end: usize,
    ) -> i32 {
        let range = end - start;
        if range == 0 {
            return -1;
        } else if end - start == 1 {
            if nums[start] == target {
                return start as i32;
            } else {
                return -1;
            }
        }
        let mid = start + (range) / 2;
        match nums[start].cmp(&nums[mid]) {
            Ordering::Equal => unreachable!(),
            Ordering::Less => match nums[start].cmp(&target) {
                Ordering::Equal => start as i32,
                Ordering::Less => match nums[mid].cmp(&target) {
                    Ordering::Equal => mid as i32,
                    Ordering::Less => {
                        Self::search_rotated_sorted_array_recursive(nums, target, mid + 1, end)
                    }
                    Ordering::Greater => {
                        Self::search_rotated_sorted_array_recursive(nums, target, start, mid)
                    }
                },
                Ordering::Greater => {
                    Self::search_rotated_sorted_array_recursive(nums, target, mid + 1, end)
                }
            },
            Ordering::Greater => match nums[mid].cmp(&target) {
                Ordering::Equal => mid as i32,
                Ordering::Less => match nums[start].cmp(&target) {
                    Ordering::Equal => start as i32,
                    Ordering::Less => {
                        Self::search_rotated_sorted_array_recursive(nums, target, start, mid)
                    }
                    Ordering::Greater => {
                        Self::search_rotated_sorted_array_recursive(nums, target, mid + 1, end)
                    }
                },
                Ordering::Greater => match nums[start].cmp(&target) {
                    Ordering::Equal => start as i32,
                    Ordering::Less => {
                        Self::search_rotated_sorted_array_recursive(nums, target, mid + 1, end)
                    }
                    Ordering::Greater => {
                        Self::search_rotated_sorted_array_recursive(nums, target, start, mid)
                    }
                },
            },
        }
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn example1() {
        let nums = vec![4, 5, 6, 7, 0, 1, 2];
        let target = 0;
        assert_eq!(Solution::search_rotated_sorted_array(nums, target), 4);
    }

    #[test]
    fn example2() {
        let nums = vec![4, 5, 6, 7, 0, 1, 2];
        let target = 3;
        assert_eq!(Solution::search_rotated_sorted_array(nums, target), -1);
    }

    #[test]
    fn example3() {
        let nums = vec![1];
        let target = 0;
        assert_eq!(Solution::search_rotated_sorted_array(nums, target), -1);
    }

    #[test]
    fn example4() {
        let nums = vec![5, 1, 3];
        let target = 3;
        assert_eq!(Solution::search_rotated_sorted_array(nums, target), 2);
    }
}
